...
#ifndef DALTON_JOB_SP_READER_H
#define DALTON_JOB_SP_READER_H
...
namespace ccio
{
    ...
    class dalton_job_sp_reader final : public job_sp_reader
    {
    public:
        dalton_job_sp_reader(
                kemiisto::io::input_stream& stream,
                boost::property_tree::ptree& root,
                ccio::molecule& molecule,
                const std::string& program_version, int printLevel,
                const ccio::dalton_mol_entry& mol_entry,
                const ccio::dalton_dal_entry& dal_entry);
        ~dalton_job_sp_reader();

    private:
        struct dalton_job_sp_reader_private;
        std::unique_ptr<dalton_job_sp_reader_private> p;

        std::vector<ccio::method_type> method_types() const override;
//        std::string method_names() const override;
